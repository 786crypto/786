...
#else
    const char* pszModule = "SevenEightSixCrypto";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SevenEightSixCrypto
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SevenEightSixCrypto
    // Mac: ~/Library/Application Support/SevenEightSixCrypto
    // Unix: ~/.SevenEightSixCrypto
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SevenEightSixCryptoData";
#else
    fs::path pathRet;
    ...
    return pathRet / "SevenEightSixCryptoData";
#else
    // Unix
    return pathRet / ".SevenEightSixCryptoData";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "SevenEightSixCrypto.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "SevenEightSixCryptod.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong SevenEightSixCrypto will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("SevenEightSixCrypto"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
